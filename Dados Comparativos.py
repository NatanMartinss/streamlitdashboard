...
WHERE appointment_specialty != 'Confirmação de Dados'
  AND DATE(CONVERT_TZ(schedule_date_time, '+00:00', '-03:00')) BETWEEN '{primeiro_dia_passado}' AND '{ultimo_dia_passado}'
UNION ALL
SELECT 
    'mês atual', COUNT(*) as quantidade
FROM appointments
WHERE appointment_specialty != 'Confirmação de Dados'
  AND DATE(CONVERT_TZ(schedule_date_time, '+00:00', '-03:00')) BETWEEN '{primeiro_dia_atual}' AND '{ultimo_dia_atual}'
"""
df_med = get_data(query_med)
df_med = df_med.set_index("periodo").loc[["mês passado", "mês atual"]].reset_index()
...
fig_med = px.bar(
    ...
    y="quantidade",
    text="quantidade",
    title="📌 Número de Atendimentos Médicos",
...
# Garantir que os dias apareçam em ordem correta
ordem_dias = ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"]
df_semana["dia"] = pd.Categorical(df_semana["dia"], categories=ordem_dias, ordered=True)
df_semana = df_semana.sort_values("dia")

# Gerar gráfico com rótulos e ordem correta das colunas
fig_semana = px.bar(
    df_semana,
    x="dia",
    y=["mes_passado", "mes_atual"],
    barmode="group",
    labels={"value": "Consultas", "dia": "Dia da Semana", "variable": "Período"},
    text_auto=True
)
...
fig_receitas.update_traces(textposition='outside')
...
fig_atestados.update_traces(textposition='outside')
st.plotly_chart(fig_atestados)
